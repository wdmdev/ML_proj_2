...
def kfold_cv(data: Data, k: int):
  ...
  weights = np.empty(shape= (k,len(lambdas),716))
  for i in range(k):
    ...
    model_weights = np.empty((len(lambdas),716))
    ...
    weights[i] = model_weights
    
  losses = losses.mean(axis=0)
  train_losses = train_losses.mean(axis=0)

  # Extract optimal weights and find n-features with highest numerical values

  weights = weights.mean(axis=0)
  weights_best = weights[np.argmin(losses)]
  weights_abs = abs(weights_best)

  n_features = 5
  features_best = np.argpartition(weights_abs, -n_features)[-n_features:]
  weight_values = weights_abs[features_best]
  print(features_best, weight_values)
  ...
  plt.scatter(lambdas,losses)
  plt.plot(lambdas, losses)
  plt.title("Test loss as function of regularization" % np.round(lambdas[np.argmin(losses)]))
  ...
  plt.grid(True)
  plt.savefig("../latex/Billeder/regularization.png")
  plt.show()
...
def run():
  ...
  data = create_dataset(feature_years, predict_years, start_year=start_year, end_year=end_year)
  log("Done creating dataset (observations, features)")
  log("Data from %s to %s was used" % (start_year, end_year))
  log("x:", data.x.shape)
  log("y:", data.y.shape)
  log()
